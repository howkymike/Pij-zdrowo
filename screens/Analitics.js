  ...
  React.useEffect(() => {
    ...
    async function GetStatistics(token) {
      ...
      const options = {
        method: "GET",
        headers: { Auth: token },
        url: `${authCtx.URL}/data/statistics`,
      };
      try {
        const response = await axios(options);
        ...
        for (let i in response.data["abnormal_data"]) {
          UpdatePH((PHArray) => [
            ...PHArray,
            response.data["abnormal_data"][i]["PH"],
          ]);
          UpdateTDS((TDSArray) => [
            ...TDSArray,
            response.data["abnormal_data"][i]["TDS"],
          ]);
          UpdateDate((DateArray) => [
            ...DateArray,
            response.data["abnormal_data"][i]["date"],
          ]);
          UpdateLocation((LocationArray) => [
            ...LocationArray,
            response.data["abnormal_data"][i]["location"],
          ]);
        }
      } catch (err) {
        console.error(err);
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <View>
          ...
          <View style={ViewText}>
            <Text style={DoubleNestedBullet}>{"\u25EF"}</Text>
          ...
          <View style={ViewText}>
            <Text style={NestedBullet}>{"\u25EF"}</Text>
            <Text style={NestedText}>
              Najlepszy stan wody odczytano w:{BestWater}
            </Text>
          </View>
          <View style={ViewText}>
            <Text style={NestedBullet}>{"\u25EF"}</Text>
            <Text style={NestedText}>
              Najgorszy stan wody odczytano w:{WorstWater}
            </Text>
          </View>
        </View>
        {myloop}
